            ...
            <div class="job-header">
               <div class="jobinfo">
                  ...
                  <div class="candidateinfo">
                     ...
                     <div class="loctext"><i class="fa fa-history" aria-hidden="true"></i> {{__('Member Since')}}, {{$user->created_at->format('M d, Y')}}
               ...
               <!-- Buttons -->
               <div class="jobButtons" style="display: none;" >
                  ...
                  <a class="hide-info" style="display: none;" href="javascript:;" onclick="send_message()" class="btn"><i class="fa fa-envelope" aria-hidden="true"></i> {{__('Send Message')}}</a>
               </div>
            </div>
           ...
           @if(!empty($user->self_intro) && file_exists('public/video/'.$user->self_intro))
                <div class="job-header">
                   <div class="contentbox">
                      <iframe height="500" width="100%" src="{{ url('public/video') }}/{{ $user->self_intro }}?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" allowfullscreen allowtransparency allow="autoplay"></iframe>
                   </div>
                </div>
            @endif
                    ...
                    <div class="col-mid-12">
                        <table class="table table-bordered table-condensed table-panding-left">
                            <tbody>
                                ...
                                @if(!empty($user->expectation_working_location))
                                <tr>
                                    <td width="50%"><span>Expectation working location</span></td>
                                    <td width="50%"><span class="text text-success">{{ $user->expectation_working_location }}</span></td>
                                </tr>
                                @endif

                                @if(!empty($user->tefl_tesol_certificate))
                                <tr>
                                    <td width="50%"><span>Have you got any TEFL/TESOL certificate?</span></td>
                                    <td width="50%"><span class="text text-success">{{ $user->tefl_tesol_certificate }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->school_you_prefer))
                                <tr>
                                    <td><span>Which type of school you prefer?</span></td>
                                    <td><span class="text text-success">{{ $user->school_you_prefer }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->students_would_you_like_teach))
                                <tr>
                                    <td><span>Which level of students would you like to teach?</span></td>
                                    <td><span class="text text-success">{{ $user->students_would_you_like_teach }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->age_range))
                                <tr>
                                    <td><span>Age Range of Students you want to teach.</span></td>
                                    <td><span class="text text-success">{{ $user->age_range }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->chinese_visa_are_you_holding))
                                <tr>
                                    <td><span>Any type of Chinese visa are you holding now? And when it expired?</span></td>
                                    <td><span class="text text-success">{{ $user->chinese_visa_are_you_holding }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->online_interview))
                                <tr>
                                    <td><span>When will you be available to attend the online interview?</span></td>
                                    <td><span class="text text-success">{{ $user->online_interview }}</span></td>
                                </tr>
                                @endif
                                @if(!empty($user->anything_else_you_want_to_remark))
                                <tr>
                                    <td><span>Anything else you want to remark?</span></td>
                                    <td><span class="text text-success">{{ $user->anything_else_you_want_to_remark }}</span></td>
                                </tr>
                                @endif
                            </tbody>
                        </table>
