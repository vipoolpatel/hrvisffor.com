...

{
...

    {
        ...
        if(empty($this->logo))
        {
            return \ImgUploader::print_image("admin_assets/no-image.png", $width, $height, '/admin_assets/no-image.png', $this->name);
        }
        else
        {
...

    {
        ...
        return CompanyStaffMessage::where('company_id', '=', $this->id)->where('status', '=', 'unviewed')->where('type', '=', 'message')->count();
...

    {
     ...
     public function countStaffMessages($id)
    {

        return CompanyStaffMessage::where('company_id', '=', $this->id)->where('user_id', '=', $id)->where('status', '=', 'unviewed')->where('type', '=', 'reply')->count();
