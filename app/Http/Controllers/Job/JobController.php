...
class JobController extends Controller
{
    ...
    {
        $this->middleware('auth', ['except' => ['jobsBySearch', 'jobDetail','jobInvitation','invitationStore','applyJobDetails']]);
    ...
    public function jobsBySearch(Request $request)
    {
        ...
        $jobs = $this->fetchJobs($search, $job_titles, $company_ids, $industry_ids, $job_skill_ids, $functional_area_ids, $country_ids, $state_ids, $city_ids, $is_freelance, $career_level_ids, $job_type_ids, $job_shift_ids, $gender_ids, $degree_level_ids, $job_experience_ids, $salary_from, $salary_to, $salary_currency, $position_looking_id, $is_featured, $order_by, $limit);
    ...
    public function jobDetail(Request $request, $job_slug)
    {
        ...
        /*         * ************************************************** */
        $seo = (object) array(
                    'seo_title' => !empty($job->getpositionlooking->name) ? $job->getpositionlooking->name : $job->title,
                    'seo_description' => $seoArray['description'],
                    'seo_keywords' => $seoArray['keywords'],
                    'seo_other' => ''
        );
    ...
    public function postApplyJob(ApplyJobFormRequest $request, $job_slug)
    {
        ...
        foreach ($request['addmore'] as $key=>$r){
            TeacherJobInterviewTime::create([
                'job_id'=>$job->id,
                'user_id'=>$user_id,
                'apply_id'=>$jobApply->id,
                'date'=>$r['date'],
    ...
    public function jobInvitation($id){
        $user = \App\User::find($id);
        $jobs = Job::where('company_id', \Illuminate\Support\Facades\Auth::guard('company')->user()->id)->get();

        return view('job.job_invitation',['user'=>$user,'jobs'=>$jobs]);
    }
    public function invitationStore(Request $request,$id){

        $row =JobInvitation::create([
    ...
    public function applyJobDetails($id){
        $row = \Illuminate\Support\Facades\DB::table('job_apply')
            ->select('job_apply.*','users.first_name','users.last_name','jobs.company_id','companies.school_id','companies.name',
                'profile_cvs.title','profile_cvs.cv_file')
            ->join('users','job_apply.user_id','=','users.id')
            ->join('jobs','job_apply.job_id','=','jobs.id')
            ->leftJoin('profile_cvs','job_apply.user_id','=','profile_cvs.user_id')
            ->leftJoin('companies','jobs.company_id','=','companies.id')
            ->where('job_apply.id',$id)
            ->where('jobs.company_id', \Illuminate\Support\Facades\Auth::guard('company')->user()->id)
            ->first();
        $teacher_availabilities = TeacherJobInterviewTime::where('apply_id',$id)->get();
        return view('school_interview.apply_details',['row'=>$row,'teacher_availabilities'=>$teacher_availabilities]);
    }
    public function applyJobDetailsByTeacher($id){
        $row = \Illuminate\Support\Facades\DB::table('job_apply')
            ->select('job_apply.*','users.first_name','users.last_name','jobs.company_id','companies.school_id','companies.name',
                'profile_cvs.title','profile_cvs.cv_file')
            ->join('users','job_apply.user_id','=','users.id')
            ...
            ->leftJoin('companies','jobs.company_id','=','companies.id')
            ->where('job_apply.id',$id)
            ->where('job_apply.user_id',\Illuminate\Support\Facades\Auth::id())
            ->first();
        $teacher_availabilities = TeacherJobInterviewTime::where('apply_id',$id)->get();
        return view('teacher_interview.apply_details',['row'=>$row,'teacher_availabilities'=>$teacher_availabilities]);
    }
