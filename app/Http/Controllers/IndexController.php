...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $latest_blogs = Blog::orderBy('id', 'desc')->where('lang', 'like', \App::getLocale())->limit(4)->get();
        $video = Video::getVideo();
        $testimonials = Testimonial::langTestimonials();

        $functionalAreas = DataArrayHelper::langFunctionalAreasArray();
        $countries = DataArrayHelper::langCountriesArraySearch();
	...
	public function beijingProfile(){
        return view('beijing_profile');
    }
	...
	public function auto_company_login(Request $request) {
        if(!empty($request->company_id)) {
            Auth::guard('company')->logout();
            $authUser = Company::where('id','=',$request->company_id)->first();
            Auth::guard('company')->login($authUser, true);
			
            if(!empty($request->message))
            {
                return redirect('company-messages');    
            }
            else
            {
