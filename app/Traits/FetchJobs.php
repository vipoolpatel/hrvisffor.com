...
trait FetchJobs
{
    ...
    private $fields = array(
        ...
        'jobs.created_at',
        'jobs.updated_at',
        'jobs.r_position_looking_id',
        'jobs.r_school_id',
        'jobs.r_work_type_id',
        'jobs.r_english_speaker_id',
        'jobs.r_visa_id',
        'jobs.r_teach_id',
        'jobs.r_position_id',
        'jobs.r_salary_id',
        'jobs.r_hour_id',
        'jobs.r_working_schedule_id',
        'jobs.r_class_size_id',
        'jobs.r_min_age_requirement_id',
        'jobs.r_max_age_requirement_id',
        'jobs.r_contact_name',
        'jobs.r_phone_number',
        'jobs.r_wechat_id',
        'jobs.r_school_name',
    ...
    public function fetchJobs($search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $position_looking_id = '', $is_featured = -1, $orderBy = 'id', $limit = 10)
    {
        ...
        {
            $query->where('jobs.r_position_looking_id' , '=' , $position_looking_id);
        }
        ...
        if(!empty(\Request::get('teacher')))
        {
            ...
            if(!empty($getuser))
            {
                /*if($getuser->r_english_speaker_id == "Yes") {
                    $query->where('jobs.r_english_speaker_id','=',$getuser->r_english_speaker_id);
                }*/
                ...
                {
                    if($getuser->r_visa_id == 1) {

                    }else{
                        $query->where('jobs.r_visa_id','=',$getuser->r_visa_id);
                    }

                }


                if(!empty(\Request::get('state')) || !empty(\Request::get('city')))
                {
                ...
                else
                {
                    ...
                    $state = !empty($getuser->state_id)?$getuser->state->state:'';
                    ...
                    if($getuser->r_teach_id != 3)
                    {
                        $query->where('jobs.r_teach_id','=',$getuser->r_teach_id);
                    }
                }
                ...
                $r_school_id = array();
                foreach($getuser->userschooljoin as $school_id) {
                    $r_school_id[] = $school_id->school_join_id;
                }
                /*if(!empty($r_school_id)) {
                    $query->whereIn('jobs.r_school_id',$r_school_id);
                } */
