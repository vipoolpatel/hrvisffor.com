...
trait ProfileEducationTrait
{
    ...
    public function showFrontProfileEducation(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileEducation)){ 
            foreach ($user->profileEducation as $education):
                ...
                $html_html .= '
                <tr id="education_' . $education->id . '">
                   <td>'.$education->r_start_date.'</td>
                   <td>'.$education->r_end_date.'</td>
                   <td>'.$education->r_school_name.'</td>
                   <td>'.$education->getCountry('country').'</td>
                   <td>'.$education->r_major.'</td>
                   <td>'.$education->r_degree.'</td>
                   <td><a href="javascript:void(0);" onclick="showProfileEducationEditModal(' . $education->id .');" class="text text-default">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_education(' . $education->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
                </tr>';
    ...
    public function showApplicantProfileEducation(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileEducation)){
            foreach ($user->profileEducation as $education):
             ...
             $html_html .= '
                <tr>
                   <td>'.date('Y-m',strtotime($education->r_start_date)).'</td>
                   <td>'.date('Y-m',strtotime($education->r_end_date)).'</td>
                   <td>'.$education->r_school_name.'</td>
                   <td>'.$education->getCountry('country').'</td>
                   <td>'.$education->r_major.'</td>
                   <td>'.$education->r_degree.'</td>
    ...
    public function storeFrontProfileEducation(ProfileEducationFormRequest $request, $user_id)
    {
        ...
        $profileEducation->r_start_date = $request->r_start_date;
        $profileEducation->r_end_date   = $request->r_end_date;
        $profileEducation->r_end_date   = $request->r_end_date;
        $profileEducation->r_school_name = $request->r_school_name;
        $profileEducation->r_major      = $request->r_major;
        $profileEducation->r_degree     = $request->r_degree;
        $profileEducation->country_id   = $request->country_id;    
        $profileEducation->save();
