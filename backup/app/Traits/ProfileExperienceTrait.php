...
trait ProfileExperienceTrait
{
    ...
    public function showFrontProfileExperience(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileExperience)) {
            foreach ($user->profileExperience as $experience):
                ...
                $html_html .= '<tr id="experience_'.$experience->id.'">
                        <td>'.$experience->r_start_date.'</td>
                        <td>'.$experience->r_end_date.'</td>
                        <td>'.$experience->r_company_name.'</td>
                        <td>'.$experience->r_position.'</td>
                        <td>'.$experience->r_title.'</td>
                        <td>'.$experience->r_duty.'</td>
                        <td>
                            <a href="javascript:void(0);" onclick="showProfileExperienceEditModal(' . $experience->id . ');" class="text text-default">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_experience(' . $experience->id . ');" class="text text-danger">' . __('Delete') . '</a>
                        </td>
                </tr>';
      ...
      //           $html .= '<div class="panel panel-info" id="experience_' . $experience->id . '">
        ...
        {
            $html_html .= '<tr>
                    <td colspan="100%">'. __('Record not found.') .'</td>
                </tr>';
        }
    ...
    public function showApplicantProfileExperience(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileExperience)) {
            foreach ($user->profileExperience as $experience):
             $html .= '<tr>
                <td>'.date('Y-m',strtotime($experience->r_start_date)).'</td>
                <td>'.date('Y-m',strtotime($experience->r_end_date)).'</td>
                <td>'.$experience->r_company_name.'</td>
                <td>'.$experience->r_position.'</td>
                <td>'.$experience->r_title.'</td>
                <td>'.$experience->r_duty.'</td>
    ...
    public function storeFrontProfileExperience(ProfileExperienceFormRequest $request, $user_id)
    {
        ...
        $profileExperience->r_start_date    = $request->r_start_date;
        $profileExperience->r_end_date      = $request->r_end_date;
        $profileExperience->r_company_name  = $request->r_company_name;
        $profileExperience->r_position      = $request->r_position;
        $profileExperience->r_title         = $request->r_title;
        $profileExperience->r_duty          = $request->r_duty;
        $profileExperience->save();
