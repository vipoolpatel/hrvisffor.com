		...
		<div class="profileban">
			...
			<div class="row">
				<div class="col-lg-12 col-md-12" style="text-align: center;">
					<a href="{{ url('jobs?teacher='.auth()->user()->id) }}" class="btn btn-danger">Match Job</a>
					<hr />
				</div>

				<div class="col-lg-12 col-md-12">
					
					<h3><i class="fa fa-bell" aria-hidden="true"></i> Messages Notification</h3>
                                  <ul class="list-group">
                                    @if(null !== ($result))
                                    <?php $data= json_decode( json_encode($result), true);
									 
									foreach($data as $notification_1){ ?>
                                    <li class="list-group-item">
										You have a new message <a href="{{route('my.updatenotification',$notification_1['id'])}}" style="margin-left:20px;" class=""> {{ $notification_1['msg'] }} @if($notification_1['seen_status']==0)
											<span class="btn btn-warning">not read</span>
										@else
                        ...
                        <div class="col-lg-7">
                            <div class="profbox">
                                ...
                                <ul class="recomndjobs">
                                    ...
                                    <li>
                                        <h4><a href="{{route('job.detail', [$match->slug])}}">{{!empty($match->getpositionlooking->name) ? $match->getpositionlooking->name : $match->title }}</a></h4>
                                        <p>{{$match->getCompany()->school_id}}</p>
                                        <p>Salary: {{ !empty($match->getexpectsalary->name) ? $match->getexpectsalary->name : '' }}</p>
                                        <p style="color: #17d27c">Location : {{ !empty($match->getLocation()) ? $match->getLocation() : '' }}</p>
                                    </li>
                                    @endforeach @endif
                                </ul>
                            </div>
