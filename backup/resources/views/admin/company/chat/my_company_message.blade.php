                              ...
                              <div class="message-inbox">
                                 ...
                                 <div class="list-wrap">
                                    <ul class="message-history">
                                       ...
                                       <li class="message-grid" id="adactive{{ $company->id}}">
                                          <a  href="javascript:;" data-gift="{{$company->id}}" id="company_id_{{$company->id}}"  onclick="show_messages('{{ $company->id}}','{{ $company->staff_id}}')">
                                             <div class="image"> 
                                                {{$company->printCompanyImage()}}
                                             </div>
                                             <div class="user-name">
                                                <div class="author"> <span>{{ $company->name}} 
                                                    @if($company->countStaffMessages($company->staff_id))
                                                        ({{$company->countStaffMessages($company->staff_id)}})
                                                    @endif
                                                </span>                       
                                                </div>
                                                @if(!empty($company->OnlineUser()))
...
<script type="text/javascript">
   function show_messages(id,staff_id)
   {
           $.ajax({
               type: "GET",
               url: "{{url('admin/company-change-message-status')}}",
               data: { 
                   'company_id': id, 
                   'staff_id' : staff_id
              },
         ...
         $.ajax({
           type: 'get',
           url: "{{ url('admin/company-append-message') }}",
           data: {
